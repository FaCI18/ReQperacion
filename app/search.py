...
def search_all_files(query: str) -> list[dict]:
    """
    Search files from ALL users with weighted relevance scoring.
    Includes the uploader's username in results.

    Args:
        query: The search query string

    Returns:
        List of dicts with file info, username, and relevance score
    """
    if not query or not query.strip():
        return []
    ...
    try:
        ...
        for row in result:
            ...
            files.append({
                ...
                "processing_status": row[7] or "pending",
                "username": row[8],
                "relevance_score": round(float(row[9]), 2) if row[9] else 0,
            })

        return files

    except Exception as e:
        logger.error(f"Search all files error: {e}")
        return _fallback_search_all(db, query)
    finally:
        db.close()


def search_user_files(user_id: int, query: str) -> list[dict]:
    """
    Search files belonging to a specific user (for the "Usuarios" tab).
    Same as search_files but returns username too.
    ...
    Returns:
        List of dicts with file info and relevance score
    """
    if not query or not query.strip():
        return []
    ...
    try:
        ...
        logger.error(f"Search user files error: {e}")
        return _fallback_search_user(db, user_id, query)
    finally:
        db.close()
...
def _fallback_search_all(db, query: str) -> list[dict]:
    """
    Fallback search across all users using LIKE.
    """
    like_pattern = f"%{query}%"
    try:
        sql = text("""
        ...
        logger.error(f"Fallback search all error: {e}")
        return []


def _fallback_search_user(db, user_id: int, query: str) -> list[dict]:
    """
    Fallback search for a specific user using LIKE.
    """
    like_pattern = f"%{query}%"
    try:
        sql = text("""
        ...
        logger.error(f"Fallback search user error: {e}")
        return []
